...
2. It uses the subnet mask to determine that the destination is in the same network.  
3. PC0 sends the packet to the switch.  
4. The switch forwards the data to PC1 using MAC addressing.  
5. PC1 receives the packet and sends a reply back.  
...
- Subnet mask is used to determine whether a destination is in the same network or not  
...
In a /24 network, the first IP represents the network, the last IP is the broadcast address, and all IPs in between are valid for devices. Devices within the same network communicate directly through a switch without requiring a router.
